...
export function HorizontalTimelineComparison({
  ...
  const { minMinutes, maxMinutes, totalMinutes } = useMemo(() => {
    ...
    // Add padding
    min -= 60; // 1 hour padding
    max += 60; // 1 hour padding
  ...
  const getPositionStyle = (task: Task) => {
    ...
    return {
      left: `${leftPercent}%`,
      width: `${Math.max(widthPercent, 0.2)}%`, // Minimum width visibility
    };
  };
        ...
        <div className="min-w-[800px] relative">
          ...
          <div className="relative h-6 mb-1">
            {days.map((day) => {
              ...
              const leftPercent =
                ((visibleStart - minMinutes) / totalMinutes) * 100;
              const widthPercent =
              ...
              return (
                <div
                  key={`day-${day}`}
                  className="absolute top-0 h-full border-l first:border-l-0 border-r border-border/50 flex items-center justify-center text-xs font-semibold text-muted-foreground bg-muted/10 truncate px-1"
                  style={{
                    left: `${leftPercent}%`,
                    width: `${widthPercent}%`,
                  }}
          ...
          <div className="relative h-6 border-b text-xs text-muted-foreground mb-4">
            {Array.from({ length: totalMinutes / 60 + 1 }).map((_, i) => {
              ...
              const leftPercent =
                ((minuteTick - minMinutes) / totalMinutes) * 100;
              if (leftPercent > 100) return null;

              // Only label every 2 hours to avoid crowding
              const showLabel = i % 2 === 0;

              return (
                <div
                  key={i}
                  className="absolute bottom-0 border-l border-border/50 h-full flex items-end pl-1 pb-1"
                  style={{ left: `${leftPercent}%` }}
                >
                  {showLabel && (
                    <span className="transform -translate-x-1/2">
                      {Math.floor((minuteTick / 60) % 24)}:00
                    </span>
                  )}
                </div>
              );
          ...
          <div className="space-y-6">
            ...
            <div className="relative group/lane">
              ...
              <div className="relative h-10 bg-muted/20 rounded border border-blue-100 overflow-hidden">
                {/* Grid Lines */}
                {Array.from({ length: totalMinutes / 60 + 1 }).map((_, i) => {
                  const minuteTick = minMinutes + i * 60;
                  const leftPercent =
                ...
                {currentTasks.map((task) => (
                  <div
                    ...
                    className="absolute top-1 bottom-1 rounded-sm shadow-sm hover:brightness-110 cursor-help transition-all hover:z-10 hover:scale-y-110"
                    style={{
            ...
            <div className="relative group/lane">
              ...
              <div className="relative h-10 bg-muted/20 rounded border border-green-100 overflow-hidden">
                {/* Grid Lines */}
                {Array.from({ length: totalMinutes / 60 + 1 }).map((_, i) => {
                  const minuteTick = minMinutes + i * 60;
                  const leftPercent =
